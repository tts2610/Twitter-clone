...
$(document).ready(function() {
    ...
    tweetArea.keypress(function(e) {
        ...
        } else if (!isCreateNormalText) { //if ((e.keyCode || e.which) == 32) 
            isHashtag = false;
            let normalText = `<span id="inputArea-normalText" style="color:black;"></span>`
            tweetArea.append(normalText);
            isCreateNormalText = true;
        }
        if (isHashtag) {

            tweetArea.html(tweetArea.children());
            $("#inputArea-hashtag").append(tweetArea.text());

        } else {
            alert(tweetArea.text());
            $("#inputArea-normalText").append(String.fromCharCode(e.keyCode));
            // tweetArea.html(tweetArea.children());
        }
    });
