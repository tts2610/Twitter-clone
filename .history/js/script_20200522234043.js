...
function ranking() {
    ...
    $("#trendingHeader").fadeOut("slow", function() { $(this).remove(); });


    $("li").each(function() {
        if ($(this).attr("id") == liId || $(this).attr("id") == "trendingHeader") {
            return false;
        }
        $(this).animate({
            "top": '+=' + moveDown
    ...
    $myLi.animate({
        ...
        $myUl.prepend(`<li class="list-group-item d-flex justify-content-between lh-condensed header" style="border-radius: 15px 15px 0px 0px;" id="trendingHeader">
    <div>
        <div class="trends-header">Trends for you</div>
    </div>
    <span class="text-muted"><i class="fal fa-cog" style="COLOR: rgba(29,161,242,1.00); font-size: larger;"></i></span>
</li>`);
